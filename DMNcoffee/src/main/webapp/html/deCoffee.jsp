...
<% if(pdto.size()<6){
 		...
 		<img class="menu_image" id="<%=pdto.get(i).getEpdnm() %>" src="image/menu/디카페인/<%=pdto.get(i).getPdnm() %>.png" data-xxx="<%= pdto.get(i).getPdno() %>">
 		<div class="menu_info" id = "<%=pdto.get(i).getEpdnm() %>Info"><%=pdto.get(i).getPdnm() %><br>
	<%= pdto.get(i).getPdprice() %>원</div>	
 	<%} %>
<%}else if(pdto.size()>=6){ 
			for(i=0;i<6;i++){%>
		<img class="menuimage" id="<%=pdto.get(i).getEpdnm() %>" src="image/menu/디카페인/<%=pdto.get(i).getPdnm() %>.png" data-xxx="<%= pdto.get(i).getPdno() %>">
 		<div class="menu_info" id = "<%=pdto.get(i).getEpdnm() %>Info"><%=pdto.get(i).getPdnm() %><br>
	<%= pdto.get(i).getPdprice() %>원</div>	
	<%} %>
<%} %>
