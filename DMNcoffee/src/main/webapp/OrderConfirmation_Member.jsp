...
<%
	...
	int orderseq = olist.get(0).getOrderseq();
	int mempoint = mdto.getMempoint();
	
	if(stampcheck.equals("true")){
		...
		<table class="check-table" border="1">
			...
			<tr>
				...
				<%
					if(stampcheck.equals("true")){
				%>
				<td class="right"><%=discountprice %>원&nbsp;</td>
				<%
					}else{
				%>
				<td class="right"><%=totalprice %>원&nbsp;</td>
			...
			<tr>
				<td colspan="2">&nbsp;스탬프 개수</td>
				<td class="right"><%=mempoint %>개&nbsp;</td>
			</tr>
		</table>
	</div>
